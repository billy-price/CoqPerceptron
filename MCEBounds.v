...
Proof.
  ...
  unfold class in H1. apply Qle_bool_iff in H1. unfold Qle. unfold Qle in H1.
  ...
  rewrite Qmult_0_r. apply Qle_refl. Qed.
...
Proof.
  ...
  { apply IHT in H. destruct T. simpl in H. rewrite Qmult_1_r in H.
    ...
    apply Qle_bool_iff in H1. apply Qmult_le_l with (z := 2#1) in H1.
    apply (Qle_trans _ _ _ H1 H). reflexivity. apply H.
...
Proof.
  ...
  { destruct p as [M' w']. inversion H1; subst. simpl. apply IHT in H3.
    apply not_correct_z_mu_neg in H2. apply (Qplus_le_compat _ 0 _ 0 H2 H3).
  } simpl in H1. inversion H1; subst. simpl. rewrite Qplus_0_r.
...
Proof.
  ...
  { destruct p as [M w]. rewrite Qvec_foil_append. assert (H0 := H).
    ...
    assert (H1 := IHE T w). apply (Qplus_le_compat _ 0 _ 0 H H1).
  } apply Qle_refl. Qed. 
...
Proof.
  intros n L. induction L; intros. simpl. repeat rewrite Qmult_0_r. apply Qle_refl.
  destruct a as [f l]. unfold Qvec_sum_normsq. fold (Qvec_sum_normsq L).
  unfold Qvec_sum_dot. fold (Qvec_sum_dot w L). simpl. repeat rewrite Qmult_plus_distr_r.
  rewrite Qmult_1_r. repeat rewrite <- Qplus_assoc. rewrite (Qplus_assoc (Qvec_sum_normsq L) _ _).
  rewrite (Qplus_comm (Qvec_sum_normsq L) _). repeat rewrite <- Qplus_assoc. rewrite Qplus_assoc.
  apply Qplus_le_compat. apply H0. left. reflexivity.
  apply IHL. apply H. intros. apply H0. right. apply H1. Qed.

Lemma Qmult_neg_le : forall (x y z : Q),
  z < 0 -> x <= y -> z * y <= z * x.
Proof.
  intros. unfold Qle. unfold Qle in H0. simpl. unfold Qlt in H.
...
Proof.
  ...
  unfold Qminus. rewrite Qopp_mult_distr_l.
  ...
  { subst. unfold max_element_normsq, min_element_product. fold (max_element_normsq T).
    fold (min_element_product w T). destruct T. apply Qle_refl.
    unfold Qge_bool. destruct (Qle_bool (max_element_normsq _) _) eqn:H0.
    destruct (Qle_bool (Qvec_dot _ _) _) eqn:H1. apply Qle_refl.
    assert (min_element_product w (p :: T) < Qvec_dot w (Qvec_mult_class l (consb f))).
    apply Qnot_le_lt. unfold not. intros. apply Qle_bool_iff in H2. rewrite H1 in H2. inversion H2.
    apply Qplus_le_compat. apply Qle_refl. apply Qlt_le_weak in H2.
  ...
  apply (Qle_trans _ _ _ H0). unfold limit_B. unfold Qminus. rewrite Qopp_mult_distr_l.
  ...
  rewrite Qopp_mult_distr_l. apply Qplus_le_compat. apply Qle_refl.
...
Proof.
  ...
  { exists (limit_B T w0). intros E. split. apply H0.
    unfold limit_B, Qminus. rewrite Qopp_mult_distr_l. assert (-(2#1) = (-2#1)). reflexivity.
    ...
    unfold Qminus. rewrite Qmult_plus_distr_r. assert (H3 := Qvec_foil_MCE_w0_le_0 E T w0).
    apply Qplus_le_compat. apply Qle_refl. rewrite <- (Qplus_0_l ((-2#1)*(Qvec_sum_dot _ _))).
    apply Qplus_le_compat. rewrite <- (Qmult_0_r (2#1)). apply Qmult_le_l. reflexivity. apply H3.
    rewrite <- Qopp_mult_distr_r. rewrite Qopp_mult_distr_l. apply Qmult_neg_le; [reflexivity | apply Qle_refl].
    ...
    rewrite Qopp_mult_distr_l in H2. apply H2.
  } exists 1. intros E. split. reflexivity. rewrite H0. simpl. assert (H1 := H0 E).
...
Proof.
  ...
  rewrite <- (Qmult_0_l C). apply Qmult_lt_r. apply (H0 E). apply (H E).
  split. apply H. rewrite (Qmult_comm B _). rewrite <- Qmult_assoc.
  rewrite (Qmult_comm _ (C * _)). apply Qmult_le_compat_r. apply (H0 E).
  apply Qlt_le_weak. apply (H E). Qed.