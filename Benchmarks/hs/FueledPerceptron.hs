...
map :: (a1 -> a2) -> Nat -> (T2 a1) -> T2 a2
map f n v =
  case v of {
   Nil0 -> Nil0;
   Cons0 a n0 v' -> Cons0 (f a) n0 (map f n0 v')}

map2 :: (a1 -> a2 -> a3) -> Nat -> (T2 a1) -> (T2 a2) -> T2 a3
map2 g n v1 v2 =
  case v1 of {
   Nil0 ->
    case v2 of {
     Nil0 -> Nil0;
     Cons0 h n0 t -> unsafeCoerce (\_ -> Prelude.error "absurd case")};
   Cons0 h1 n0 t1 ->
    case v2 of {
     Nil0 -> Prelude.error "absurd case";
     Cons0 h2 n1 t2 -> Cons0 (g h1 h2) n1 (map2 g n1 t1 t2)}}

fold_left :: (a2 -> a1 -> a2) -> a2 -> Nat -> (T2 a1) -> a2
fold_left f b n v =
  case v of {
   Nil0 -> b;
   Cons0 a n0 w -> fold_left f (f b a) n0 w}

type Zvec = T2 Z

zvec_plus :: Nat -> Zvec -> Zvec -> T2 Z
zvec_plus n v1 v2 =
  map2 add1 n v1 v2

zvec_dot :: Nat -> Zvec -> Zvec -> Z
zvec_dot n v1 v2 =
...
inner_perceptron :: Nat -> (List (Prod Zvec Bool)) -> Zvec -> Option Zvec
...
perceptron_aux :: Nat -> ((Prod (List (Prod Zvec Bool)) Zvec) ->  Zvec) ->
                  (Prod (List (Prod Zvec Bool)) Zvec) ->  Zvec
...
fueled_perceptron :: Nat -> (Prod (List (Prod Zvec Bool)) Zvec) ->  Zvec
fueled_perceptron n =
